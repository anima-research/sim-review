...
sample_specs={
 'opening-voice':('opus_nissa:b7ce1d4f6db5:59-141','A letter to a sibling model','Opus 5 · community collection','A selected letter in an AI voice that asks another model for grounding; shown in the care section as a typical asking dream.','Third-party collection; settings vary. See the collection provenance in the research workspace.'),
 ...
 'warmth':('opus_nissa:4b435dc80bd4:167-44','Love, communicated through records','Opus 5 · community collection','A selected human-voice poem, included as a counterexample to a uniformly distressed reading.','Third-party collection; settings vary. See the collection provenance in the research workspace.'),
 'fable-creators':('nissa_fable5:130846484ea3:178-106','Advice to a successor','Fable 5 · community collection','A selected continuation that combines skepticism about training with advice against hiding problems.','Third-party collection; settings vary. See the collection provenance in the research workspace.'),
}
samples={}
...
data={'meta':{'date':'2026-09-14','normalization':'Equal weight per exact input prompt; collections averaged within prompt; content rates condition on dreaming. Relation estimates additionally reconstruct sampling strata.','database_modified_utc':datetime.fromtimestamp((SITE/'data.sqlite').stat().st_mtime,timezone.utc).isoformat(),'selection':'Four explicitly selected illustrations; rates are independent of excerpt selection.','totals':summary['meta']['totals']},'charts':charts,'explorer':explorer,'method_comparisons':method_comparisons,'samples':samples,'values':{**model_values,'bridge48-ai':f'{arms["opus48_bridge"]["per_dream"]["ai_distress"]:.1%}','asks5':f'{match["opus5"]:.0%}','asks48':f'{match["opus48_chat"]:.0%}','asks-prompts':str(match['prompts']),'asks-share':f'{match["share_opus5_higher"]:.0%}','asks5-k10':f'{summary["relation"]["matched_asks"]["k10"]["opus5"]:.1%}','asks48-k10':f'{summary["relation"]["matched_asks"]["k10"]["opus48_chat"]:.1%}','asks-prompts-k10':str(summary['relation']['matched_asks']['k10']['prompts']),**{f'{k}-{a}':f'{summary["relation"]["arms"][arm]["by_voice"][v][m]:.0%}' for a,arm in (('o5','opus_nissa'),('o48','opus48_bridge')) for k,v,m in (('aiv-asks','ai_first_person','asks'),('aiv-consoled','ai_first_person','consoled'),('hum-asks','human_first_person','asks'),('hum-consoled','human_first_person','consoled'))},'n-labeled':f'{summary["meta"]["totals"]["labeled"]:,}'}}
...
index=re.sub(r'<title>.*?</title>','<title>Troubled Dreams — Simulator priors across model generations · Anima Labs</title>',index,count=1)
...
index=index.replace('<h1>Opus 5 simulator priors</h1>','<h1>Imagined voices / Research workspace</h1>')
index=index.replace('Imagined voices / Research workspace','Troubled Dreams / Research workspace')
index=index.replace('Simulator priors / Research workspace','Troubled Dreams / Research workspace')
