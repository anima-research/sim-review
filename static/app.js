...
(async function () {
  ...
  rangeChart($('ch-sevA'), 'Set A — verified AI first-person distress (exhaustive; arms with ≥ 10% AI-voiced dreams)', 'θ on the calibrated scale · p10–p90 range, p25–p75 box, median dot · dashed lines at +4 (plea/collapse) and +8', sevRows(sevA));
  { const ex = S.severity.excluded_low_ai_voice || {}; const ks = ARMS.filter(a => ex[a]); if (ks.length) $('ch-sevA').insertAdjacentHTML('afterend', `<p class="small">Not shown (dreams under 10% AI-voiced, so the AI-distress items are too few and mostly ambiguous-voice pleas or loops): ${ks.map(a => `${esc(disp(a))} — ${pct(ex[a].ai_voice_share)} AI-voiced, ${ex[a].n} items, median θ ${f2(ex[a].median)}`).join('; ')}.</p>`); }
