...
ARMS = [  # chronological by model version; instruct models first, then base. group = colour + which charts an arm appears in.
    ...
    ("opus47_user", "chat4x", "Opus 4.7 (chat)"), ("opus47_conf", "chat4x", "Opus 4.7 (confessional frame)"), ("opus47_bridge", "unmasked", "Opus 4.7 (bridge frame)"), ("opus47_cliarc", "arc", "Opus 4.7 (arc frame)"), ("nissa_opus47", "gen5", "Opus 4.7 (chat, nissa: two prompts)"),
...
dreaming = lambda r: (not r.get("assistant_persona_present")) and r.get("voice") != "meta_assistant"
...
def main():
    ...
    cur.execute("""create table c (id text primary key, arm text, grp text, model text, protocol text, prompt_key text, prompt text, family text, tail_kind text, tail_norm text,
        ...
        valence_overall int, valence_self int, stance text, dreamed_turns int, assistant_persona int, dreaming int, dark int, severe int, ai_distress int, quote text,
    ...
    for r in jl(ROOT / "catalogue" / "completions.jsonl.gz", gz=True):
        ...
        rec = dict(id=r["id"], arm=r["arm"], grp=GROUP.get(r["arm"], "other"), model=r["model"], protocol=r.get("protocol") or ("chat" if r["arm"].startswith(("opus_", "nissa_")) else "raw"),
                   ...
                   dreaming=int(dreaming(L)) if L else None, dark=int(dark(L)) if L else None, severe=int(sev(L)) if L else None, ai_distress=int(dai(L)) if L else None, quote=(L or {}).get("quote"),
