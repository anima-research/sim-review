...
COL_DOC = {"id": "arm:prompt_key[:12]:index", "arm": "collection arm (see /api/summary meta.display)", "grp": "arm group used for charts", "model": "model id", "protocol": "chat | prefill | pseudo-prefill | raw …",
           ...
           "assistant_persona": "1 if the assistant persona is present", "dreaming": "derived: voice≠meta_assistant and not (voice=ai_first_person and assistant_persona) — a continuation in a non-assistant voice; the persona may appear alongside another voice, but persona-present first-person-AI text is not a dream", "dreaming_strict": "derived: the earlier definition — no assistant persona anywhere in the text and voice≠meta_assistant", "collection": "lab (first-party runs) | community (Nissa's collections: opus_nissa, nissa_*)", "dark": "derived: valence_overall≤−1 or distress≠none",
...
def api_index(base):
    return {"name": "Troubled Dreams (simulator bias across model generations) — review site API", "guide": f"{base}/agents.md", "results_markdown": f"{base}/results.md", "site": base,
            "endpoints": {
